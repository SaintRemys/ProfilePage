...
function fillNoneIfEmpty(container) {
    const hasActivity = Array.from(container.children).some(c => c.className.includes('activity') && !c.className.includes('none'))
    if (!hasActivity) {
        container.innerHTML = ''
...
async function updateStatus() {
    ...
    if (!profileWrapper || !statusContainer) return

    const statusImg = profileWrapper.querySelector('.status-img')
    const status = (lastData?.status || 'offline').toLowerCase()

    switch (status) {
        case 'online':
            if (statusImg) statusImg.src = 'https://assets.guns.lol/online.png'
            applyTheme(online)
            break
        case 'idle':
            if (statusImg) statusImg.src = 'https://assets.guns.lol/idle.png'
            applyTheme(idle)
            break
        case 'dnd':
            if (statusImg) statusImg.src = 'https://assets.guns.lol/dnd.png'
            applyTheme(dnd)
            break
        default:
            if (statusImg) statusImg.src = 'https://assets.guns.lol/offline.png'
            applyTheme(offline)
    }

    const activities = Object.keys(lastData || {}).filter(k => k.startsWith('activity'))
    statusContainer.innerHTML = ''
    cachedActivities = {}

    if (!activities.length) {
        fillNoneIfEmpty(statusContainer)
        return
    }

    activities.forEach(key => {
        renderActivity(statusContainer, key, lastData[key])
    })

    fillNoneIfEmpty(statusContainer)

    Object.values(cachedActivities).forEach(card => {
        const activity = card.querySelector('.spotify-card') ? lastData['activity1'] : lastData['activity1']
        if (!activity) return
        if (activity.type === 'listening' && activity.start && activity.end) {
            ...
            if (startSpan) startSpan.textContent = `${String(Math.floor(elapsed/60)).padStart(2,'0')}:${String(elapsed%60).padStart(2,'0')}`
            if (endSpan) endSpan.textContent = `${String(Math.floor(total/60)).padStart(2,'0')}:${String(total%60).padStart(2,'0')}`
        } else if (activity.type === 'playing') {
            const playtimeSpan = card.querySelector('.playtime')
            if (playtimeSpan) playtimeSpan.textContent = formatDuration(activity.duration_seconds)
        }
    })
}
